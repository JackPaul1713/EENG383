        ...
        if ((EUSART1_is_rx_ready()))
        {
            ...
            switch (cmd)
            {
                ...
                case 'm':
                    ...
                    i = 2;
                    tsum = transmitBuffer[0] + transmitBuffer[1];
                    ...
                    {
                        if(i > 32 -2) break;
                        transmitBuffer[i] = character;
                        tsum += character;
                        i++;
                        printf("%c", character);
                        character = EUSART1_Read();
                    }
                    transmitBuffer[i] = '\0'; i++;
                    transmitBuffer[i] = tsum;

                    printf("\n\rCreated\n\r  message: %s\n\r  checksum: %u\n\r  source address: %u\n\r  destintation address: %u\n\r", (transmitBuffer + 2), tsum, transmitBuffer[0], transmitBuffer[1]);
                    break;
                case 's':

                    printf("Enter source address, hit return when done.\n\r");

                    transmitBuffer[0] = read8bit();
                ...
                case 'R':
                    if(received == 1)
                    {
                        printf("Received\n\r  message: %s\n\r  checksum: actual(%u), expected(%u)\n\r  source address: %u\n\r  destintation address: %u\n\r", (receiveBuffer + 2), rsum, rcsum, receiveBuffer[0], receiveBuffer[1]);
                        received = 0;
                    }
                ...
                case 'M':
                    ...
                    while(character == RCREG1)
                    {
                        if(received == 1)
                        {
                            printf("  %u %u %u %u    %s\n\r", receiveBuffer[0], receiveBuffer[1], rsum, rcsum, (receiveBuffer + 2));
                            received = 0;
                        }
                    }
                ...
                case 'x':
                    ...
                    printf("%u: %x %c\n\r", i, transmitBuffer[i], transmitBuffer[i]); i++;
                    printf("%u: %x %c\n\r", i, transmitBuffer[i], transmitBuffer[i]);
                    break;
                case 'X':
                    i = 0;
                    ...
                    printf("%u: %x %c\n\r", i, receiveBuffer[i], receiveBuffer[i]); i++;
                    printf("%u: %x %c\n\r", i, receiveBuffer[i], receiveBuffer[i]);
                    break;
                case 'r':
                    RCSTA2bits.CREN = 0;
    ...
    if(transmitting == 1)
    {
        switch(state)
        {
            ...
            case TXDATABITS:
                ...
                if ((transmitBuffer[i] & mask) != 0) EPWM2_LoadDutyValue(0);
                else EPWM2_LoadDutyValue(25);
                mask = mask << 1;
                if(mask == 0) state = TXSTOPBIT;
                break;
            case TXSTOPBIT:

                EPWM2_LoadDutyValue(0);
                if((transmitBuffer[i-1] == '\0') && (i > 0))
                {
                    i = 0;
                    transmitting = 0;
                }
    ...
    if(received == 0)
    {
        switch (state)
        {
            case RXDATA:
                ...
                receiveBuffer[i] = character;
                if(i == 0) rsum = character;
                else rsum += character;
                i++;
                if((character == '\0') || (i == 32 -1)) state = RXCSUM;
                break;
            case RXCSUM:
                printf(">> RxCSum: %u\n\r", character);
