            ...
            switch (cmd)
            {
                ...
                case 'f':
...

                    {
                        ...
                        {
                            printf("S[%u] ", i * 16);
                            if(i*16 < 100) printf(" "); if(i*16 < 10) printf(" ");
                            for(j = 0; j < 16; j++)
                            {
                                sample = samples[(i * 16) + j];
                                printf("%u ", sample);
                                if(sample < 100) printf(" "); if(sample < 10) printf(" ");
                            }
                            printf("\n\r");
                        }
                        printf("\n\r");
...

                    {
                        ...
                        for(i = 1; i < ci; i++)
                        {
                            ...
                            totPeriod += period;
                            printf("%u - %u = %u\n\r", crossings[i], crossings[i-1], period);
                        }
                        printf("\n\r\n\r");
                        avgPeriod = (totPeriod * 25) / (ci-1);
                        frequency = ((ci-1) * 1000000/25) / totPeriod;
...
void sampleTMR0ISR(void)
{
    ...
    {
        case MICIDLE:
            if(sampling == 1) state = MICTRIGGER;
            break;
        case MICTRIGGER:
            if((ADRESH > (128 + threshold)) || (ADRESH < (128 - threshold))) state = MICACQUIRE;
            break;
        case MICACQUIRE:
            if(sampling == 0) state = MICIDLE;
            sampled = 1;
            break;
    }
    ADCON0bits.GO_NOT_DONE = 1;

    TMR0_WriteTimer(0x10000 - (400 - 75));
