...
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.40\\pic\\include\\c99\\inttypes.h" 1 3
# 13 "C:\\Program Files\\Microchip\\xc8\\v2.40\\pic\\include\\c99\\inttypes.h" 3
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.40\\pic\\include\\c99\\bits/alltypes.h" 1 3
# 13 "C:\\Program Files\\Microchip\\xc8\\v2.40\\pic\\include\\c99\\inttypes.h" 2 3
...
uint8_t tour = 0;
uint8_t rgbDuty[3] = {0xFF, 0xFF, 0xFF};
uint8_t rgbDirection[3] = {0x00, 0x00, 0x00};
uint8_t edgeLocation = 0x00;
uint8_t tourDirectionIndex = 0;
    ...
    uint8_t rgbTourStart[3] = {0x00, 0xFF, 0x00};
    uint8_t tourDirections[3][3] = {{0x01, 0xFF, 0x00}, {0xFF, 0x00, 0x01}, {0x00, 0x01, 0xFF}};
...
void main(void)
{
    ...
    EPWM1_LoadDutyValue(rgbDuty[0]);
    EPWM2_LoadDutyValue(rgbDuty[1]);
    EPWM3_LoadDutyValue(rgbDuty[2]);
        ...
        if ((EUSART1_is_rx_ready()))
        {
            ...
            switch (cmd)
            {
                ...
                case 'z':
                    ...
                    break;
                case 'R':
                    if(rgbDuty[0] > 0x00) rgbDuty[0]--;
                    break;
                case 'G':
                    if(rgbDuty[1] > 0x00) rgbDuty[1]--;
                    break;
                case 'B':
                    if(rgbDuty[2] > 0x00) rgbDuty[2]--;
                    break;
                case 'r':
                    if(rgbDuty[0] < 0xFF) rgbDuty[0]++;
                    break;
                case 'g':
                    if(rgbDuty[1] < 0xFF) rgbDuty[1]++;
                    break;
                case 'b':
                    if(rgbDuty[2] < 0xFF) rgbDuty[2]++;
                    break;
                case 'C':

                    rgbDuty[0] = rgbTourStart[0];
                    rgbDuty[1] = rgbTourStart[1];
                    rgbDuty[2] = rgbTourStart[2];

                    rgbDirection[0] = tourDirections[tourDirectionIndex][0];
                    rgbDirection[1] = tourDirections[tourDirectionIndex][1];
                    rgbDirection[2] = tourDirections[tourDirectionIndex][2];
...
void TMR0ISR(void)
{
    if(tour)
    {
        ...
        if(edgeLocation != 0xFF)
        {

            rgbDuty[0] += rgbDirection[0];
            rgbDuty[1] += rgbDirection[1];
            rgbDuty[2] += rgbDirection[2];
            edgeLocation++;
        }
        else
        {
            edgeLocation = 0x00;
            tourDirectionIndex = (tourDirectionIndex + 1) % 3;
            rgbDirection[0] = tourDirections[tourDirectionIndex][0];
            rgbDirection[1] = tourDirections[tourDirectionIndex][1];
            rgbDirection[2] = tourDirections[tourDirectionIndex][2];
        }
