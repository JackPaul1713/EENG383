...
uint16_t duration[3][12] = {{15000, 15000, 60000, 15000, 15000, 60000, 30000, 15000, 15000, 15000, 60000, 15000},
                                   {30000, 30000, 30000},
                                   {30000, 15000, 30000, 60000}};
            ...
            switch (cmd)
            {
                ...
                case 'r':
                    ...
                    for (uint8_t i = 0; i < songLength; i++)
                    {
                        ...
                        mintime = duration[songSelection][i] - 5000;
                        maxtime = duration[songSelection][i] + 5000;
...
void myTMR0ISR(void)
{
    ...
    switch (tmr0State)
    {
        ...
        case PLAY_NOTE:
            ...
            break;
        case PLAY_REST:
            if((playSong == 1) && (noteIndex < songLength-1))
            {

                playNote = 0;
                noteIndex++;
                tmr0State = PLAY_NOTE;
                TMR0_WriteTimer(0x10000 - 20000);
            }
            else
            {
                playNote = 0;
...
void myTMR1ISR(void)
{
    ...
    static uint16_t scale[30] = {18192, 17168, 16192, 15296, 14448, 13632, 12864, 12144, 11472, 10816, 10208, 9632, 9088, 8592, 8112, 7648, 7232, 6816, 6432, 6080, 5728, 5408, 5104, 4816, 4560, 4288, 4048, 3824, 3616, 3408, 3216, 256};
    static uint8_t notes[3][12] = {{28, 8, 0, 28, 8, 28, 0, 26, 28, 8, 0, 28},
                                           {6, 29, 22},
                                           {2, 11, 23, 21}};
