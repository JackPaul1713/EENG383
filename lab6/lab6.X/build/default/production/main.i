            ...
            switch (cmd)
            {
                ...
                case 'm':
                    ...
                    while(letter != '\r')
                    {
                        ...
                        {
                            i--;
                            tsum -= transmitBuffer[i];
                        }
                        else
                        {
                            if(i > 32 -2) break;
                            transmitBuffer[i] = letter;
                            tsum += letter;
                            i++;
                        }
                        printf("%c", letter);
                    ...
                    printf("\n\rCreated\n\r  message: %s\n\r  checksum: %u\n\r", transmitBuffer, tsum);
                    break;
                case 'S':
                    i = 0;
                    while(transmitBuffer[i] != '\0')
                    {
                        transmitCharacterOverIR(transmitBuffer[i], bitPeriod[baudRateSelected]);
                        i++;
                    }
                    transmitCharacterOverIR('\0', bitPeriod[baudRateSelected]);
                    transmitCharacterOverIR(tsum, bitPeriod[baudRateSelected]);
                    printf("Tranmitted\n\r  message: %s\n\r  checksum: %u\n\r", transmitBuffer, tsum);
                    break;
                case 'R':
                    if(received == 1)
                    {
                        printf("Received\n\r  message: %s\n\r  checksum: actual(%u), expected(%u)\n\r", receiveBuffer, rsum, rcsum);
                        received = 0;
                    }
